...
def optimize(frame):
    ...
    if 'deadline_date' in frame.columns:
        frame['deadline_date'] = frame['deadline_date']\
            .apply(lambda x: str(datetime.fromtimestamp(int(float(x)) / pow(10, 3)))[:10] if x != '' else '')
...
class Migrate2Sql(Teams):
    ...
    def __init__(self, location, clickup_api_token=None, attribute_values=None, spaces=None, optimise=False,
                 sql_connection=None, dtype=None):
        """
        Constructor for Request class
